...
tune_res %>% collect_metrics(summarize = TRUE) %>% 
  filter(.metric == "rmse") %>% 
  summarize(mean = mean(mean)) # Get p.value for LASSO model ????
```
...
2. We would go with the orinary least square model, as it has lower RMSE than the LASSO model. The predict accuracies of two models seem not to be reliable as the RMSE values for both models are quite high, considering the scale of the response variable.

3.They currently don't show any visible harms. Given the nature of the dataset, there seems to be no caution when we communicate our work.
