...
gam_output <- Titanic_clean %>%
  bind_cols(predict(gam_mod, new_data = titanic_r)) %>%
  mutate(resid = Fare - .pred)

ggplot(gam_output, aes(x = Age, y = resid)) +
  geom_point() +
  geom_smooth() +
  geom_hline(yintercept = 0, color = "red") +
  theme_classic()

ggplot(gam_output, aes(x = Parch, y = resid)) +
  geom_point() +
  geom_smooth() +
  geom_hline(yintercept = 0, color = "red") +
  theme_classic()
...
ggplot(gam_output, aes(x = .pred, y = resid)) +
  geom_point() +
  geom_smooth() +
  geom_hline(gam_output = 0, color = "red") +
  theme_classic()
```
